...
compute(pass){}}
class SwordTip extends Standard2DVertexObject{constructor(device,canvasFormat){let vertices=new Float32Array([0,.8,.05,.7,-.05,.7,-.05,.7,-.05,.05,.05,.05,-.05,.7,.05,.7,.05,.05]);super(device,canvasFormat,vertices,'../lib/Shaders/optimized_SwordBlade.wgsl','triangle-strip');this._vertices=vertices;}}
class Lines extends Standard2DVertexObject{constructor(device,canvasFormat){let vertices=new Float32Array([0,0.8,0,.05,0,.8]);super(device,canvasFormat,vertices,'../lib/Shaders/optimized_SwordHilt.wgsl','line-strip');this._vertices=vertices;}}
class SwordHilt extends Standard2DVertexObject{constructor(device,canvasFormat){let vertices=new Float32Array([-.15,.05,-.1,.05,-.1,0,-.1,.05,-.1,0,.1,0,-.1,.05,.1,.05,.1,0,.15,.05,.1,.05,.1,0]);super(device,canvasFormat,vertices,'../lib/Shaders/optimized_SwordHilt.wgsl','triangle-strip');this._vertices=vertices;}}
class SwordHandle extends Standard2DVertexObject{constructor(device,canvasFormat){let vertices=new Float32Array([-.025,0,-.025,-.2,.025,-.2,-.025,0,.025,0,.025,-.2,-.025,-.2,0,-.225,.025,-.2]);super(device,canvasFormat,vertices,'../lib/Shaders/optimized_SwordHandle.wgsl','triangle-strip');this._vertices=vertices;}}
class BackgroundRays extends Standard2DVertexObject{constructor(device,canvasFormat){let vertices=new Float32Array([0,.4,-1,.4,-1,-.6,0,.4,1,.4,1,1,0,.4,0,1,-.6,1,0,.4,0,-1,1,-1]);super(device,canvasFormat,vertices,'../lib/Shaders/optimized_BackgroundRays.wgsl','triangle-strip');this._vertices=vertices;}}
